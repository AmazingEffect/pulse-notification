...
public class TraceGrpcClientAspect {
     ...
     * @throws Throwable - 예외
     */
    @Around(value = "@annotation(com.pulse.notification.config.trace.annotation.TraceGrpcClient) && args(id, context)", argNames = "joinPoint,id,context")
    public Object traceGrpcClient(ProceedingJoinPoint joinPoint, Long id, Context context) throws Throwable {
        // 1. Span 생성
        Span span = tracer.spanBuilder("Notification [grpc] client request")
                .setParent(context)
                .startSpan();
